...
roomNumber = 6
interval = 15 #in seconds roughly between measurements

myThingspeakAPI = "DA1K0TYF36CAE516"  #your key from your own thingspeak account. Put yours here.
...
# create CSV headings for data
f= open("VentCheckSpreadsheetData.csv", "a" , newline="")
csver = csv.writer(f)
csver.writerow(["Time","Date","R"+str(roomNumber)+" Temp", "Outdoor Temp", "Diff in Temp"])
f.close()

def updateThingSpeak(outT,inT,diffT): 
   #print('Updating thingspeak') 
   baseURL = 'https://api.thingspeak.com/update?api_key=%s' % myThingspeakAPI  

       
   fi = urlopen(baseURL + "&field1=%s" % (str(outT)) + "&field2=%s" % (str(inT)) + "&field3=%s" % (str(diffT))) 
   #print (fi.read()) 
   fi.close()      

   #print('Ooops... Bad thingspeak..lets try again...') 
...
def readLocalTemp():
    ...
    while True:
        ...
        dhtHumidity, dhtTemperature = DHT.read_retry(sensor, pin)
...
def writeTempToCSV(temperatureForCSV):
    f= open("VentCheckSpreadsheetData.csv", "a" , newline="")    
    csver = csv.writer(f)
    csver.writerow([timeNow, dateNow,localTempReading, outsideTempDownload, tempDifference])     
    f.close()       
...
try:
    ...
    while True:
        ...
        # THingspeak upload
        updateThingSpeak(outsideTempDownload,localTempReading,tempDifference)
        ...
        # turn off DHT power until next time
        pb.output.e.write(0)
        
        # wait: uses output leds as countdown lights
        for x in range(interval): # 3600 is 1 hour, 900 is 15 mins, set at top of program
            pb.output.f.write(1)
            sleep(0.33)
            pb.output.g.write(1)
            sleep(0.33)
