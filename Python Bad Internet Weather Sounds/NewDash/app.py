...
def measure_speed():
    """
    Runs speedtest-cli, appends results to externalSpeeds.csv,
    with DayOfWeek, Hour for consistent data
    """
...
def get_typical_averages(df):
    """
    Group by (DayOfWeek, Hour) => average Download.
    Returns dict: {("Sunday",2): 500.1, ("Monday",9): 45.3, ...}
    """
...
def school_map():
    ...
    # Make the chart more likely to show
    # in case the user wants to see the background
    fig.update_layout(
...
def fantasy_map():
    ...
    # Make a fantasy-themed feedback
    ...
    # Make the chart background transparent, plus font=Uncial Antiqua
    fig.update_layout(
        title="External Download Speed (Today)",
        xaxis_title="Time",
        yaxis_title="Mbps",
