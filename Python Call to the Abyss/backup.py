...
from mysecrets import API_KEY  # Import your API key from secrets.py

# Initialize the OpenAI client using the API key from secrets.py.
...
def fade_out():
    """
    Gradually reduces the video playback speed to 0.5x and fades out the music volume.
    Plays a crash sound ("crash.ogg") at the start.
    Does not perform a fade-to-black; simply stops the video.
    """
    ...
    # Now, in the main thread, close any OpenCV windows.
    print("Closing OpenCV windows using cv2.destroyAllWindows()...")
...
def call_to_abyss(current_level, outcome_table):
    ...
    system_message = (
        ...
        "Describe in the second person tense, in bullet points 5 ways how the player's could be is affected by this outcome."
    )
    user_message = wish
    ...
    # Fade out the video speed and fade out the audio.
    fade_out()
