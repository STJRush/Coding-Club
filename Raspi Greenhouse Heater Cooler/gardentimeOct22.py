...
from apds9960 import APDS9960
    ...
    # this lets us have a time delay (see line 15)  
GPIO.setmode(GPIO.BCM)     # set up BCM GPIO numbering  

GPIO.setup(14, GPIO.OUT)    # red led
GPIO.setup(15, GPIO.OUT)   # green led
GPIO.setup(18, GPIO.OUT)    # blue led
#LED GROUND BOARD-PIN-14 

GPIO.setup(19, GPIO.IN, pull_up_down=GPIO.PUD_UP)    # FAN TEST SWITCH
GPIO.setup(26, GPIO.IN, pull_up_down=GPIO.PUD_UP)    # RELAY TEST SWITCH

GPIO.setup(23, GPIO.OUT)    # set GPIO25 as FAN
GPIO.setup(24, GPIO.OUT)   # set GPIO24 other FAN pin

GPIO.setup(25, GPIO.OUT)   # set GPIO25 as RELAY for Heater
...
def create_CSV():
    ...
    now = datetime.now() 
    path = now.strftime("%Y%m%d_%H%M") +"gardenData.csv" 
    ...
    print("Will save as gardenData.csv")

def update_CSV():
    print("Saving to csv") #your file name, will create or overwrite.
    now = datetime.now()
    path = "gardenData.csv" 
...
try:
    ...
    while True:
        ...
        print("Waiting 60 seconds, Test Button Available...")
        
        for x in range(600): # wait a minute
...
except:          # this block will run no matter how the try block exits
    print("Exiting")
    GPIO.cleanup()  
